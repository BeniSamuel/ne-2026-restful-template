...
const DashLeft = () => {
  ...
  const displayName = currentUser ? `${currentUser.firstName} ${currentUser.lastName}` : "Safety User";
  const initials = currentUser ? `${currentUser.firstName[0]}${currentUser.lastName[0]}` : "FS";
        ...
        >
          ...
          {!collapsed ? (
            <div className="min-w-0">
              ...
              <p className="text-xs font-semibold text-white/65">{currentUser?.role ?? "USER"}</p>
            </div>
          ) : null}
        </div>
